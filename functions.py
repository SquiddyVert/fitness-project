...
#     def setinputComplete(set_frame):
#         """checking if data is saved, if not, retrieve them from entries and store into set instance of the exercise object"""
#         # checks if set has been saved already
#         if saved["status"]:  
#             return
...
#         # Create a new Set instance
#         newSet = Set(reps=reps, weight=weight , frame = set_frame)
...
#         #configure delete button to actually work as intended once there is data in the entry fields
#         delete_set_button.config(command=lambda: delete_set_row(GUI, set_frame, exercise_frame, newSet))
#         # Unbind theevents to prevent further triggers
#         weightEntry.unbind("<FocusOut>")


#     # def save_last_set(GUI):
#     #     """Force save the last set when focus is lost or when the user is done entering data."""
#     #     if exercise.sets:
#     #         last_set = exercise.sets[-1]
#     #         if not last_set.saved["status"]:  # Check if the last set was saved
#     #             setinputComplete(last_set.frame)  # Force save the last set

    
#     weightEntry.bind("<FocusOut>", lambda event: setinputComplete(set_frame))
 
# def save_all_sets(GUI):
#     """Force save all current entries."""
#     for exercise in GUI.currentWorkout.exercises: #iterates through all the exercises in the workout
#         for set_instance in exercise.sets:  # Iterate over all sets in the exercise
#             for widget in set_instance.frame.winfo_children():      # Get the Entry widgets from the set frame
#                 if isinstance(widget, ttk.Entry):
#                     widget.event_generate("<FocusOut>")  # forces the FocusOut event
...
# def save_workout_p1(GUI):
#     """force saves all the sets even if the entry boxes are not unfocused"""
#     save_all_sets(GUI)

# #
# def save_workout_p2(GUI):
#     """Save the current workout and sets to the data list and display them in the table."""
#     if not GUI.currentProfile:
#         tkinter.messagebox.showerror("Error", "Please select a profile before saving")
#         return
...
# #function combines two functions so that one button press runs two functions
# def save_workout_full(GUI):
#     save_workout_p1(GUI)
#     save_workout_p2(GUI)
